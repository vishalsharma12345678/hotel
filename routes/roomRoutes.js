...
router.get("/lockRoom/:id",async function (req, res) {
  await Room.findByIdAndUpdate({ _id: req.params.id },{$set:{Room_Status:"Locked"}}).then((room) => {
    if (!room) return res.status(404).send("Error creating room");
    res.send("ok");
  });
});
...
router.get("/unlockRoom/:id", function (req, res) {
    Room.findByIdAndUpdate({ _id: req.params.id },{$set:{Room_Status:"Available"}}).then((room) => {
      if (!room) return res.status(404).send("Error creating room");
...
router.post("/getroombyid", async(req, res) => {
    try {
         const room = await Room.findOne({'_id' : req.body.roomid}).populate('currentbookings').exec()
         res.send(room)
    } catch (error) {
...
router.get("/getallroomsAvalible", async(req, res) => {
    try {
         const rooms = await Room.find({Room_Status:"Available"}).populate('currentbookings').exec()
         res.send(rooms)
    } catch (error) {
         return res.status(400).json({ message: error });
    }
...
router.get("/roomsDetails", async(req, res) => {
  try {
       let EmptyRooms = await Room.find({status:"booked"})
       ...
       res.send({EmptyRooms:`${EmptyRooms.length}`,OccupiedRoom:`${OccupiedRoom.length}`,Upcomingbooking:`${Upcomingbooking.length}`,Lastbooking:`${Lastbooking.length}`})
  } catch (error) {
       return res.status(400).json({ message: error });
  }
});
